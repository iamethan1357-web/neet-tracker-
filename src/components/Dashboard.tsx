  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="flex items-start justify-between">
        ...
        <motion.div
          className="text-right border border-gray-100 rounded-xl px-4 py-2"
      ...
      <motion.div
        variants={item}
        className="border border-gray-100 rounded-xl p-4 bg-gradient-to-r from-gray-50 to-white relative overflow-hidden"
      >
        <div className="absolute top-2 right-3 text-3xl opacity-5">🩺</div>
        <p className="text-sm text-gray-600 italic leading-relaxed">&ldquo;{getDailyQuote()}&rdquo;</p>
      </motion.div>

      {/* Stats Row */}
      <motion.div variants={item} className="grid grid-cols-3 gap-3">
        <motion.div
          className="border border-gray-100 rounded-xl p-4 text-center card-3d"
          whileHover={{ y: -3 }}
        >
          <motion.div
        ...
        <motion.div
          className="border border-gray-100 rounded-xl p-4 text-center card-3d relative"
          whileHover={{ y: -3 }}
        >
        ...
        <motion.div
          className="border border-gray-100 rounded-xl p-4 text-center card-hover"
          whileHover={{ y: -3 }}
        >
          <div className="text-2xl font-bold">{longestStreak}</div>
          <div className="text-[10px] text-gray-500 mt-1">Best</div>
        </motion.div>
      </motion.div>
      ...
      <motion.div variants={item} className="flex justify-center">
        <motion.div
          className="text-2xl opacity-10 inline-block"
      ...
      <motion.div variants={item}>
        ...
        <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden">
          <motion.div
            className={`h-3 rounded-full ${progress === 100 ? "bg-emerald-500" : "bg-black"}`}
            initial={{ width: 0 }}
        ...
        <div className="grid grid-cols-2 gap-3">
          {subjects.map((s, i) => (
            <motion.button
              ...
              className={`p-4 rounded-xl border transition-all text-left relative overflow-hidden card-3d ${
                log[s.key]
            ...
            >
              {log[s.key] && (
                <motion.div
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="absolute top-2 right-2 w-5 h-5 bg-emerald-500 rounded-full flex items-center justify-center text-[10px] animate-tick"
                >✓</motion.div>
              )}
      ...
      {/* Questions */}
      <motion.div variants={item} className="border border-gray-100 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          <div>
            <div className="text-sm font-medium flex items-center gap-1.5">🧠 Questions Practiced</div>
            <div className="text-[10px] text-gray-500">Every question counts towards your rank</div>
          </div>
          <motion.div
            className="text-2xl font-bold tabular-nums"
            key={log.questionsPracticed}
        ...
        <div className="flex gap-2">
          {[10, 25, 50, 100].map((n) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex-1 py-2.5 text-xs border border-gray-200 rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-colors font-medium"
            >
              +{n}
            </motion.button>
          ))}
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="py-2 px-3 text-xs border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-gray-400"
          >
            ↺
          </motion.button>
        </div>
      ...
      {/* Water Intake */}
      <motion.div variants={item} className="border border-gray-100 rounded-xl p-4 relative overflow-hidden">
        <div className="absolute -bottom-4 -right-4 text-6xl opacity-5 rotate-12">💧</div>
        <div className="flex items-center justify-between mb-3">
          <div>
            <div className="text-sm font-medium">💧 Hydration Tracker</div>
            <div className="text-[10px] text-gray-500">Target: 4L/day — Stay hydrated, future doc!</div>
          </div>
          <motion.div
        ...
        <div className="w-full bg-blue-50 rounded-full h-4 overflow-hidden mb-3">
          <motion.div
            className="bg-gradient-to-r from-blue-300 to-blue-500 rounded-full h-4"
            initial={{ width: 0 }}
            animate={{ width: `${Math.min((log.waterIntake / 4) * 100, 100)}%` }}
        ...
        <div className="flex gap-2">
          {[0.25, 0.5, 1].map((n) => (
            <motion.button
              ...
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="flex-1 py-2.5 text-xs border border-blue-100 rounded-xl hover:bg-blue-50 transition-colors font-medium text-blue-600"
            >
              +{n}L
            </motion.button>
          ))}
      ...
      <motion.div variants={item} className="grid grid-cols-2 gap-3">
        <motion.div whileHover={{ y: -2 }} className="border border-gray-100 rounded-xl p-4">
          <div className="text-sm font-medium mb-1 flex items-center gap-1">😴 Sleep</div>
          <div className="text-[10px] text-gray-500 mb-3">Target: 7-8h for peak cognition</div>
          <input
            type="number" min="0" max="12" step="0.5" value={log.sleepHours}
            onChange={(e) => updateField("sleepHours", parseFloat(e.target.value) || 0)}
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-black focus:ring-1 focus:ring-black/5"
          />
          {log.sleepHours > 0 && log.sleepHours < 7 && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-[10px] text-amber-500 mt-1.5">
              ⚠ Sleep more for better memory retention
            </motion.p>
          )}
        </motion.div>
        <motion.div whileHover={{ y: -2 }} className="border border-gray-100 rounded-xl p-4">
          <div className="text-sm font-medium mb-1 flex items-center gap-1">🏫 Coaching</div>
          <div className="text-[10px] text-gray-500 mb-3">Default: 6 hours</div>
          <input
            type="number" min="0" max="12" step="0.5" value={log.coachingHours}
      ...
      <motion.div variants={item}>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            <motion.div
              key={s.key}
              className="border border-gray-100 rounded-xl p-3 card-hover"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="space-y-2">
                <div>
                  <div className="text-[9px] text-gray-400 mb-1">Hours</div>
                  <input
                    type="number" min="0" max="12" step="0.5" value={log[s.key] as number}
                    onChange={(e) => updateField(s.key, parseFloat(e.target.value) || 0)}
                    className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-xs focus:outline-none focus:border-black"
                  />
                </div>
                <div>
                  <div className="text-[9px] text-gray-400 mb-1">Questions</div>
                  <input
                    type="number" min="0" step="1" value={log[s.qKey] as number}
      ...
      <AnimatePresence>
        {(saving || justSaved) && (
          ...
          >
            <div className={`text-xs px-4 py-2 rounded-full shadow-lg ${justSaved ? "bg-emerald-500 text-white" : "bg-black text-white"}`}>
