    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      <motion.div variants={item} className="flex gap-2">
        <motion.button onClick={handleExportPDF} disabled={exporting || !profile} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="flex-1 py-3.5 bg-black text-white rounded-xl text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <motion.div variants={item} className="flex gap-2">
        {(["study", "tests"] as SubView[]).map((v) => (
          <motion.button key={v} onClick={() => setSubView(v)} whileTap={{ scale: 0.95 }}
            className={`px-4 py-2 rounded-xl text-xs font-medium transition-all ${subView === v ? "bg-black text-white shadow-md" : "bg-gray-100 text-gray-600"}`}>
      ...
      {subView === "tests" && (
        <>
          <motion.div variants={item} className="flex items-center justify-between">
            ...
            <motion.button onClick={() => setShowTestForm(true)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="text-xs bg-black text-white px-3 py-1.5 rounded-xl">+ Add Test</motion.button>
          </motion.div>
          {showTestForm && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="border border-gray-200 rounded-xl p-4 space-y-3 overflow-hidden">
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-[10px] text-gray-500 uppercase">Date</label><input type="date" value={newTest.date} onChange={(e) => setNewTest({...newTest, date: e.target.value})} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm" /></div>
                <div><label className="text-[10px] text-gray-500 uppercase">Total Score</label><input type="number" value={newTest.score} onChange={(e) => setNewTest({...newTest, score: parseInt(e.target.value)||0})} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm" /></div>
              </div>
              <div className="grid grid-cols-4 gap-2">
                {(["physicsScore","chemistryScore","botanyScore","zoologyScore"] as const).map((k) => (
                  <div key={k}><label className="text-[9px] text-gray-500 uppercase">{k.replace("Score","")}</label><input type="number" value={newTest[k]} onChange={(e) => setNewTest({...newTest,[k]:parseInt(e.target.value)||0})} className="w-full px-2 py-1.5 border border-gray-200 rounded text-xs" /></div>
                ))}
              </div>
              <div><label className="text-[10px] text-gray-500 uppercase">Mistakes</label><textarea value={newTest.mistakes} onChange={(e) => setNewTest({...newTest, mistakes: e.target.value})} rows={2} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm resize-none" placeholder="Weak areas..." /></div>
              <div className="flex gap-2">
                <motion.button onClick={addTest} whileTap={{ scale: 0.95 }} className="px-4 py-2 bg-black text-white rounded-xl text-xs">Save</motion.button>
                <motion.button onClick={() => setShowTestForm(false)} whileTap={{ scale: 0.95 }} className="px-4 py-2 bg-gray-100 rounded-xl text-xs">Cancel</motion.button>
              </div>
            </motion.div>
          )}
          ...
          <div className="space-y-2">
            {tests.map((t, i) => (
              <motion.div key={t.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                className="border border-gray-100 rounded-xl p-4 card-hover">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="text-lg font-bold">{t.score}<span className="text-sm text-gray-400">/{t.totalMarks}</span></div>
                    <div className="text-xs text-gray-500">{t.date}</div>
                  </div>
                  <motion.button onClick={() => deleteTest(t.id)} whileTap={{ scale: 0.8 }} className="text-gray-300 hover:text-red-500 text-sm">✕</motion.button>
                </div>
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {[{l:"Phy",v:t.physicsScore},{l:"Chem",v:t.chemistryScore},{l:"Bot",v:t.botanyScore},{l:"Zoo",v:t.zoologyScore}].map((s) => (
                    <div key={s.l} className="text-center bg-gray-50 rounded-lg py-1.5"><div className="text-xs font-medium">{s.v}</div><div className="text-[9px] text-gray-400">{s.l}</div></div>
                  ))}
                </div>
                {t.mistakes && <div className="mt-2 pt-2 border-t border-gray-100"><div className="text-[10px] text-gray-400 uppercase mb-1">Mistakes</div><div className="text-xs text-gray-600">{t.mistakes}</div></div>}
              </motion.div>
            ))}
            {tests.length === 0 && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12"><div className="text-4xl mb-3 animate-float">📝</div><p className="text-gray-400 text-sm">No test records yet</p></motion.div>}
          </div>
        </>
      )}
    </motion.div>
