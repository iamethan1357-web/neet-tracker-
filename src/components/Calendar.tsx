    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => setYear(year - 1)} className="w-8 h-8 border border-gray-200 rounded-xl text-sm flex items-center justify-center hover:bg-gray-50">←</motion.button>
      ...
      <AnimatePresence>
        {selectedDate && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} className="border border-gray-200 rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-medium text-sm flex items-center gap-1.5">📋 {selectedDate}</h4>
              <motion.button whileTap={{ scale: 0.9 }} onClick={() => setSelectedDate(null)} className="text-gray-400 hover:text-black text-sm">✕</motion.button>
            </div>
            {selected ? (
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div className="col-span-2 pt-2 border-t border-gray-100 mt-1 space-y-1.5 text-gray-600">
